...
export const TownfolkDialogueOverlay: React.FC<TownfolkDialogueOverlayProps> = ({
  ...
  const questions = CHARACTER_PRESET_QUESTIONS[npc.id] || [
  ...
  const handleAskQuestion = async (questionText: string) => {
    ...
    try {
      const response = await townfolkVoice.askQuestion(npc.id, trimmed);
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {questions.map((q, idx) => (
              <button
                ...
                className="px-2.5 py-1 text-xs font-serif bg-stone-950 hover:bg-amber-950/80 disabled:opacity-50 text-amber-200/90 hover:text-amber-100 border border-amber-900/60 hover:border-amber-600 rounded-lg transition-colors shadow-sm text-left"
              >
                &ldquo;{q}&rdquo;
              </button>
            ))}
